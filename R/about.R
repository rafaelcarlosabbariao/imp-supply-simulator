  ...
  tagList(
    ...
    tags$div(id = "mc2-howto",
      mc2_panel(title = "How to use it", icon = "list-ol",
        tags$p(class = "mc2-muted",
          ...
          "that step and every step after it."),
        tags$div(class = "mc2-steps",
          .about_step(1, "Study Configuration",
            "Check or edit the enrollment plan and dosing schedule, or upload your own (.csv, .xls, .xlsx). Choose how many trials to simulate and press Run Enrollment."),
          .about_step(2, "Visits & Demand",
            "Set the visit window and the simulation end date, then press Run Visit Simulation. The chart shows units dispensed per DU per month."),
          .about_step(3, "Supply & Inventory",
            "Set the as-of date and the planning assumptions: lead time, safety stock, order-up-to target, unplanned-visit uplift and oversupply. Keep the sample inventory or upload your own, then press Run Inventory Projection."),
          .about_step(4, "Portfolio",
            "Counts of stockout, at-risk and OK site × DUs, with a rollup for each study."),
          .about_step(5, "Site Map",
            "Sites coloured by status. Click a site to see its DUs and its inventory over time. The feed on the right lists live supply-chain headlines."))),
        ...
        tags$table(class = "table table-sm mc2-cols",
          ...
          tags$tbody(
            ...
            tags$tr(tags$td("Site and depot inventory"),
                    tags$td("Protocol, site or depot, DU, quantity, retest or expiry date, lot.",
    ...
    mc2_panel(title = "Limits of this build", icon = "triangle-exclamation",
      tags$ul(class = "mc2-limits",
        ...
        tags$li("The app runs every arm at a fixed dose and does not ship a startup stock to sites before",
                "their first patient. Dose titration and site seeding exist in the engine and in",
                tags$code("scripts/run_simulation.R"), "."),
